...
class DomainDisentangleExperiment:  # See point 2. of the project
    ...
    def train_iteration(self, data_source, data_target):
        ...
        # 2: domain clr with feature from obj encoder
        self.freezeLayer(self.model.domain_classifier, True)
        # no calculation of loss
        l_class_ent_1 = self.entropy_loss(DCfcs)
        l_class_ent_1.backward(retain_graph=True)
        self.updateOpt()
        ...
        l_domain_1.backward(retain_graph=True)

        # 4: obj clr with feature from dom encoder
        self.freezeLayer(self.model.category_classifier, True)
        l_domain_ent_1 = self.entropy_loss(Cfds)
        # l_domain_ent_1.backward(retain_graph=True)
        self.updateOpt()
